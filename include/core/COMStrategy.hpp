...
public:
     ...
     * @param game Global game reference for dice/tile context.
     * @return `true` when the purchase should be attempted.
     */
    virtual bool shouldBuy(Property* property, Player& player, logic::Game& game) const noexcept = 0;

    /**
     * @brief Produce a maximum bid increment for auction resolution.
     * @param property Lot being auctioned.
     * @param player Acting COM instance.
     * @param game Global game reference.
     * @param previous_bid Last acknowledged bid from another participant.
     * @return Non-negative bid cap for this COM.
     */
    virtual int bidAmount(Property* property, Player& player, logic::Game& game, int previous_bid) const noexcept = 0;
     ...
     */
    virtual bool shouldBuild(Property* property, Player& player, logic::Game& game) const noexcept = 0;
     ...
     */
    virtual bool shouldMortgage(Property* property, Player& player, logic::Game& game) const noexcept = 0;
     ...
     */
    virtual int suggestChoice(const std::string& context, int defaultIndex, int optionCount) const noexcept;
};
