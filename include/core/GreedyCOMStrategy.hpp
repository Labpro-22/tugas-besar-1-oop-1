...
public:
     ...
     */
    bool shouldBuy(Property* property, Player& player, logic::Game& game) const noexcept override;

    /**
     * @brief Raise bids up to `min(balance, previous*1.25 + 50)`.
     */
    int bidAmount(Property* property, Player& player, logic::Game& game, int previous_bid) const noexcept override;

    /**
     * @brief Build whenever the next house cost is affordable.
     */
    bool shouldBuild(Property* property, Player& player, logic::Game& game) const noexcept override;

    /**
     * @brief Mortgage only when the treasury is already empty/negative.
     */
    bool shouldMortgage(Property* property, Player& player, logic::Game& game) const noexcept override;
     ...
     */
    int suggestChoice(const std::string& context, int defaultIndex, int optionCount) const noexcept override;
};
