...
class CardDeck {
 ...
 private:
   ...
   * @note Extension (not in spec): implements reshuffling discard pile inside
   * `draw()` for readability.
   */
  void reshuffleDiscard();

  std::vector<std::unique_ptr<T>>
      cards_;  // Extension (not in spec): spec lists `vector<T*>`; deck owns
               // via `unique_ptr`.
...
// Note: implementation is in the header file to avoid writing out
// CardDeck<SomeT>
