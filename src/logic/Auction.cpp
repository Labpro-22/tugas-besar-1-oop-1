...
Auction::Auction(core::Property* subject, const std::vector<core::Player*>& eligiblePlayers)
    : subject_(subject), participants_(eligiblePlayers), currentBid_(0), currentWinner_(nullptr) {}
...
bool Auction::placeBid(core::Player& p, int amount) {
    if (amount <= currentBid_) {
        throw InvalidMoveException("Tawaran M" + std::to_string(amount) + " harus lebih tinggi dari M" + std::to_string(currentBid_));
    }
...
core::Player* Auction::getCurrentWinner() const {
