...
GameStateDTO SaveLoadManager::load(const std::string& filename) const {
        ...
        throw InvalidConfigException{filename, "board size on line 1"};
    {
        ...
        ss >> dto.boardSize;
        if (ss.fail())
            throw InvalidConfigException{filename, "integer board size"};
    }
    ...
    for (int i = 0; i < numPlayers; ++i) {
        ...
        if (ss.fail())
            throw InvalidConfigException{filename,
                "player row: NAME BALANCE POSITION STATUS IS_COM"};
    ...
    for (int i = 0; i < numProperties; ++i) {
        ...
        prop->code       = code;
        prop->id         = -1;  // resolved saat GameLoader::applyDTO via Board::getTileByCode
    ...
    // Populate ownedPropertyIds per player (derived, bukan authoritative)
