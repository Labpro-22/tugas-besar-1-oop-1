...
void SaveLoadManager::save(const GameStateDTO& dto,
    ...
    for (const auto* prop : dto.properties) {
        ...
        if (prop->ownerName.empty()) propStatus = "BANK";
        else if (prop->isMortgaged)  propStatus = "MORTGAGED";
        else                         propStatus = "OWNED";
    ...
    for (const auto& card : dto.skillDeckOrder)
        file << card << "\n";

    // Transaction log
    file << dto.log.size() << "\n";
    for (const auto& entry : dto.log) {
        file << entry.turnNumber  << " "
             << entry.playerName  << " "
             << entry.actionType  << " "
             << entry.description << "\n";
    }
...
GameStateDTO SaveLoadManager::load(const std::string& filename) const {
    ...
    for (int i = 0; i < numPlayers; ++i) {
        if (!std::getline(file, line))
            throw InvalidConfigException{
                filename, "player state row " + std::to_string(i + 1)};

        PlayerDTO          player;
        std::istringstream ss(trimCR(line));
        std::string        status;
        int                isCOM = 0;
        ss >> player.name >> player.balance >> player.positionCode >> status >> isCOM;
        ...
        for (int j = 0; j < numCards; ++j) {
            if (!std::getline(file, line))
                throw InvalidConfigException{
                    filename, "card row for player " + player.name};
    ...
    for (int i = 0; i < numProperties; ++i) {
        if (!std::getline(file, line))
            throw InvalidConfigException{
                filename, "property row " + std::to_string(i + 1)};
        ...
        if (!prop->ownerName.empty())
            ownerToIndices[prop->ownerName].push_back(i);
    ...
    for (auto& player : dto.players) {
        ...
        if (it != ownerToIndices.end())
            player.ownedPropertyIds = it->second;
    }
    ...
    for (int i = 0; i < numDeckCards; ++i) {
        if (!std::getline(file, line))
            throw InvalidConfigException{
                filename, "skill deck card " + std::to_string(i + 1)};
    ...
    for (int i = 0; i < numLogEntries; ++i) {
        if (!std::getline(file, line))
            throw InvalidConfigException{
                filename, "log entry " + std::to_string(i + 1)};
