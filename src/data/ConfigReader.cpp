...
ConfigReader::ConfigReader(const std::string& basePath)
    : basePath_(basePath) {}
...
std::vector<PropertyConfig*> ConfigReader::readProperties() {
    ...
    while (std::getline(file, line)) {
        ...
        if (type == "STREET") {
            ...
            cfg->type  = type;
            cfg->color = color;
            ss >> cfg->purchasePrice
               >> cfg->mortgageValue
               >> cfg->houseUpgradeCost
            ...
            if (ss.fail()) {
                ...
                throw InvalidConfigException{
                    "property.txt", "complete STREET row for " + code};
            }
            ...
            if (ss.fail()) {
                for (auto* c : configs) delete c;
                throw InvalidConfigException{
                    "property.txt", "complete RAILROAD row for " + code};
            }
            configs.push_back(cfg);
            ...
            if (ss.fail()) {
                for (auto* c : configs) delete c;
                throw InvalidConfigException{
                    "property.txt", "complete UTILITY row for " + code};
            }
            configs.push_back(cfg);
...
TaxConfig ConfigReader::readTax() {
    ...
    if (!file.is_open())
        throw InvalidConfigException{"tax.txt", "readable file"};
...
SpecialConfig ConfigReader::readSpecial() {
    ...
    if (ss.fail())
        throw InvalidConfigException{
            "special.txt", "two integers: GO_SALARY JAIL_FINE"};
    return cfg;
}
...
MiscConfig ConfigReader::readMisc() {
    ...
    if (ss.fail())
        throw InvalidConfigException{
            "misc.txt", "two integers: MAX_TURN SALDO_AWAL"};
    return cfg;
}
