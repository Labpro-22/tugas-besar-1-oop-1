...
std::string ConfigReader::filePath(const std::string& filename) const {
    ...
    static const std::set<std::string> boardSpecific = {"property.txt", "aksi.txt"};
...
std::vector<PropertyConfig*> ConfigReader::readProperties() {
    ...
    if (!file.is_open())
        throw InvalidConfigException{"property.txt",
            "readable file at " + filePath("property.txt")};
    ...
    while (std::getline(file, line)) {
        ...
        if (type == "STREET") {
            ...
            cfg->id = id; cfg->code = code; cfg->name = name;
            cfg->type = type; cfg->color = color;
            ss >> cfg->purchasePrice >> cfg->mortgageValue
               >> cfg->houseUpgradeCost >> cfg->hotelUpgradeCost;
            ...
            cfg->id = id; cfg->code = code; cfg->name = name;
            cfg->type = type; cfg->color = color; cfg->purchasePrice = 0;
            int dummy;
...
TaxConfig ConfigReader::readTax() {
    ...
    if (ss.fail())
        throw InvalidConfigException{"tax.txt",
            "three integers: PPH_FLAT PPH_PERSENTASE PBM_FLAT"};
    return cfg;
}
...
BoardConfig ConfigReader::readBoardConfig() {
    ...
    int tileCount = 0;
    try { tileCount = std::stoi(trimCR(line)); }
    catch (...) {
        throw InvalidConfigException{"board.txt", "integer tile count on first line"};
    }
