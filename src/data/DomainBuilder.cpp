...
std::unique_ptr<core::Property> DomainBuilder::buildProperty(
  ...
  if (cfg.getType() == "STREET") {
    ...
    return std::make_unique<core::Street>(
        cfg.name, cfg.purchasePrice, cfg.mortgageValue, cg, rents,
        sc.houseUpgradeCost, sc.hotelUpgradeCost);
  }
  ...
  throw InvalidConfigException(
      "property type", "STREET|RAILROAD|UTILITY, got '" + cfg.getType() + "'");
}
...
std::unique_ptr<core::Tile> DomainBuilder::buildActionTile(
  ...
  if (cfg.tileType == "SPESIAL") {
    ...
    if (cfg.code == "PEN")
      return std::make_unique<core::JailTile>(pos, cfg.name);
    if (cfg.code == "BBP")
      return std::make_unique<core::FreeParkingTile>(pos, cfg.name);
    if (cfg.code == "PPJ")
      return std::make_unique<core::GoToJailTile>(pos, cfg.name);
  ...
  if (cfg.tileType == "PAJAK") {
    if (cfg.code == "PPH")
      return std::make_unique<core::TaxTile>(
          pos, cfg.name, taxCfg.incomeTaxFlat, taxCfg.incomeTaxPercent,
          core::TaxType::PPH);
    if (cfg.code == "PBM")
      return std::make_unique<core::TaxTile>(
          pos, cfg.name, taxCfg.luxuryTaxFlat, 0, core::TaxType::PBM);
...
std::vector<TileEntry> DomainBuilder::buildBoard(
    ...
    const std::vector<ActionTileConfig>& actionCfgs, const TaxConfig& taxCfg) {
