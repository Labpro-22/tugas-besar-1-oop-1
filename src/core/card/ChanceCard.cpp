...
ChanceCard::ChanceCard(std::string description,
...
std::unique_ptr<ActionCard> ChanceCard::makeMoveBack(int spaces,
                                                     std::string description) {
  return std::unique_ptr<ActionCard>(new ChanceCard(
      std::move(description), [spaces](Player& player, GameContext& ctx) {
...
std::unique_ptr<ActionCard> ChanceCard::makePayBank(int amount,
                                                    std::string description) {
  return std::unique_ptr<ActionCard>(new ChanceCard(
...
std::unique_ptr<ActionCard> ChanceCard::makeGoToJail(std::string description) {
  return std::unique_ptr<ActionCard>(new ChanceCard(
      ...
      [](Player& player, GameContext& ctx) { ctx.sendToJail(player); }));
}

std::unique_ptr<ActionCard> ChanceCard::makeAdvanceTo(int index,
                                                      std::string description) {
  return std::unique_ptr<ActionCard>(new ChanceCard(
      std::move(description), [index](Player& player, GameContext& ctx) {
        const int n = ctx.getBoardSize();
        if (n <= 0) {
          return;
        }
...
std::unique_ptr<ActionCard> ChanceCard::makeAdvanceToNearestRailroad(
    std::string description) {
  return std::unique_ptr<ActionCard>(new ChanceCard(
      std::move(description), [](Player& player, GameContext& ctx) {
        const int n = ctx.getBoardSize();
        if (n <= 0) {
          return;
        }
