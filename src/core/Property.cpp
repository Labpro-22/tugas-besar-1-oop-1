...
Property::Property(const string& name, int price, int mortgageValue)
    : name_(name),
      owner_(nullptr),
      isMortgaged_(false),
      price_(price),
...
void Property::mortgage() {
  if (!isOwned()) {
    throw InvalidMoveException(
        "Tidak bisa menggadaikan properti yang belum dimiliki");
  }
  if (isMortgaged_) {
    throw InvalidMoveException("Properti '" + name_ +
                               "' sudah dalam status tergadai");
  }
  isMortgaged_ = true;
}
...
void Property::unmortgage() {
  if (!isMortgaged_) {
    throw InvalidMoveException("Properti '" + name_ +
                               "' tidak sedang digadaikan");
  }
  isMortgaged_ = false;
}

void Property::setOwner(Player* p) { owner_ = p; }

Player* Property::getOwner() const { return owner_; }
...
int Property::getPrice() const { return price_; }
int Property::getMortgageValue() const { return mortgageValue_; }
...
int Property::getFestMultiplier() const { return festMultiplier_; }

int Property::getFestDuration() const { return festDuration_; }
